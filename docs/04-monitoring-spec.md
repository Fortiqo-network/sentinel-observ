...
3. **Per-probe rules**:
   - Timeout: **10 s** for direct probes (aggregate needs headroom for its internal 3 s fan-out).
   - **UP** = HTTP 2xx/3xx within timeout. **DOWN** = anything else (5xx, 4xx, timeout, DNS/conn error).
   - **In-run retry**: a failed probe is retried up to **2 more times, 5 s apart, in the same run**. Only 3-of-3 failures count as DOWN. This kills one-off network blips without delaying detection to the next 5-min tick — worst-case detection is still ≤ ~6 min from actual failure.
...
| Case | Behaviour |
|---|---|
| Slack API itself fails | Alert is retried on the next tick (a `pending_alerts` flag on the incident row); alerts are never silently dropped |
| First run ever (no previous state) | Baseline written, no alerts — avoids a false alarm burst at install time |
